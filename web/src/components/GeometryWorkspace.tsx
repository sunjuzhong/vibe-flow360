      ...
      <aside className="geometry-entity-panel">
        <div className="geometry-panel-heading">
          ...
          <span className="geometry-count-badge">
            {(manifest?.groups.length ?? 0) + (manifest?.edges?.length ?? 0) + parameterEntities.length}
          </span>
        </div>
      ...
      <aside className="geometry-review-panel">
        ...
        {activeCapabilityPanel === 'health' && (
          ...
          <div className="geometry-panel-intent">
            ...
            <span>{t('Each item below states the available evidence. Unknown means the synchronized resource does not expose enough data for that check; it is not silently treated as passed.')}</span>
          </div>
