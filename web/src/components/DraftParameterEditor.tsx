...
export function draftAIClarificationMessage(action: AgentAction, t: (text: string) => string = (text) => text): string {
  const lines = action.message.trim() ? [action.message.trim()] : []
  for (const question of action.questions ?? []) {
    const details = [
      question.field ? `${t('Field')}: ${question.field}` : '',
      question.message,
      question.reason ?? '',
      question.urgency ? `${t('Urgency')}: ${question.urgency}` : '',
      question.type ? `${t('Type')}: ${question.type}` : '',
      question.unit ? `${t('Unit')}: ${question.unit}` : '',
      question.min !== undefined ? `${t('Minimum')}: ${question.min}` : '',
      question.max !== undefined ? `${t('Maximum')}: ${question.max}` : '',
      question.placeholder ? `${t('Placeholder')}: ${question.placeholder}` : '',
      question.default !== undefined ? `${t('Default')}: ${draftAIClarificationValue(question.default)}` : '',
      question.options?.length
        ? `${t('Options')}: ${question.options.map((option) => option.label ? `${option.label} (${option.value})` : option.value).join(', ')}`
        : '',
    ].filter(Boolean)
    if (details.length) lines.push(details.join(' — '))
  }
  for (const warning of action.warnings ?? []) {
    if (warning.trim()) lines.push(`${t('Warning')}: ${warning.trim()}`)
  }
  for (const assumption of action.assumptions ?? []) {
    if (assumption.trim()) lines.push(`${t('Assumption')}: ${assumption.trim()}`)
  }
