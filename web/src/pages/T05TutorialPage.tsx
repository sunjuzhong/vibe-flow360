  ...
  const lesson = () => {
    switch (step.id) {
      ...
      case 'evidence': return <>
        ...
        <div className={`lesson-callout ${reviewed.length === t05Evidence.length ? 'success' : ''}`}>{reviewed.length === t05Evidence.length ? <CheckCircle2 size={17}/> : <AlertTriangle size={17}/>}<p><strong>{reviewed.length}/5 reviewed</strong>{reviewed.length === t05Evidence.length ? 'The wake-mesh comparison contract is ready.' : 'Review every mesh requirement before creating the experiment.'}</p></div>
      ...
      default: return <>
        ...
        <div className="run-readiness-card"><div className="run-ready-icon"><CheckCircle2 size={30}/></div><div><span>CONFIGURATION STATUS</span><strong>Two wake strategies validated</strong><p>Both Drafts request the same center-plane mesh slice; corridor length and crossflow spacing are the controlled differences.</p></div></div>
        <TutorialTransferCheck items={t05Pedagogy.transferQuestions}/>
        <TutorialEnvironmentBuilder status={status} tutorialId="T05" defaultProjectName="Tutorial T05 · cylinder wake refinement" heading="Build the T05 wake-mesh environment" description="The app uploads the bundled cylinder geometry, registers the Sphere, Box, Cylinder, and Slice entities, and creates compact-wake and focused-wake VolumeMesh Drafts." configurationSummary="Registered near-body sphere, structured wake box, axisymmetric core, directional spacing, center-plane slice" draftKind="VolumeMesh" baselineValue="Compact wake regions" variantValue="Focused wake corridor" successDescription="Both Draft entity catalogs and VolumeMesh parameter sets are synced. No surface or volume mesh computation has been submitted." createEnvironment={createT05Environment}/>
      </>
    }
  }
