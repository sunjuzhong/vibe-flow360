...
export const t02Steps: TutorialStep[] = [
  ...
  { id: 'run', label: '06', title: 'Create the Web environment', summary: 'Upload a mesh and create two configured Drafts.' },
]
...
export function t02DraftPatch(entry: T02Entry, alpha: 0 | 5): Record<string, unknown> {
  const template = caseTemplate as unknown as Record<string, unknown>
  const condition = template.operating_condition as Record<string, unknown>
  const baseline = { operating_condition: condition }
...
export async function createT02Environment(
  ...
  const shared = { source_id: rootId }
  const drafts = await Promise.all([
    client.createConfiguredDraft(projectId, { ...shared, name: 'T02 baseline · α 0°', patch: t02DraftPatch(input.entry, 0) }),
    client.createConfiguredDraft(projectId, { ...shared, name: 'T02 variant · α 5°', patch: t02DraftPatch(input.entry, 5) }),
  ])
  onStage('ready')
  return { projectId, rootResourceId: rootId, baselineDraft: drafts[0], variantDraft: drafts[1] }
}
