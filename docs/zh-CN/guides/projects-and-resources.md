...
如果 schema 加载失败，可以继续使用 JSON，并稍后重试加载 Form schema。如果校验或保存失败，
请保持编辑器打开：候选稿和撤销历史仍保留在本地，点击**重试验证**或**重试保存**会重新提交
同一版本。不要为了恢复网络连接而放弃改动。

键盘用户可以使用方向键、Home 和 End 在 Form 与 JSON 之间切换。验证摘要操作会把焦点移到
相关字段。未保存关闭对话框会将焦点限制在内部，直到用户作出选择；继续编辑时焦点会返回
关闭按钮。
